...
| `mdb.c`, `midl.c`, `lmdb.h`, `midl.h` | copied from the vendored OpenLDAP LMDB tree at `lmdb/lmdb/libraries/liblmdb/`, pinned commit `7ed58053` (mdb.master3 / LMDB 1.0 line) with `mingw-win32-fixes.patch` applied — the same tree all shipped native binaries are built from. |
...
from the pinned+patched tree as part of the native build run. Monocypher and
the shim/amalgamation are maintained manually.
